## IN SCOPE
- The `env-marker` category in `set-leakscan`: what identity it resolves, from where, and how it matches
- Diff-added-only measurement for that category
- Commit-time refusal of staged content and of commit messages, regardless of remote visibility
- Hook wiring for both actors: git `pre-commit`/`commit-msg` via an installer, and the agent-side PreToolUse gate on `git commit`
- Loud reporting of identity sources that could not be resolved
- Deliberate exceptions through the existing allow file

## OUT OF SCOPE
- Retrospective specification of the pre-existing push-time checks (consumer names, credentials, home paths, phones) — separate change
- Scrubbing already-committed history that carries the name or username (the user's call, not the gate's)
- Deploying commit hooks to consumer projects via `set-project init`
- The local-config layer itself (see `local-config`)
