## IN SCOPE
- One resolution chain for machine- and project-specific values: env var, project file, machine file, default
- Storage under the set-core config directory — outside every repository, never committed
- A CLI face (`set-config get|set|list`) over the same chain
- Value masking in listings, and file permissions on stored values

## OUT OF SCOPE
- Orchestration directives (`orchestration.yaml`, `load_config_file`) — unchanged
- The registry (`projects.json`) and existing per-subsystem files (`discord.json`, `leakscan-allow.txt`) — they stay where they are; migrating them is a separate decision
- Secret management beyond file permissions (no encryption, no agent storage)
- Deploying any template that writes local-config files into consumer projects
