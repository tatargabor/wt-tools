...
- `lib/wt_orch/verifier.py:136-174` — `_load_web_security_rules()` for retry context

## What Changes

### 1. Implement `is_done("test")` in loop_tasks.py
- Add "test" case to `is_done()` that runs the project's test command
- Test command source: `directives.test_command` from orchestration state, or auto-detected from package.json/pytest.ini
