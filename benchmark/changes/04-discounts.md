...
1. **Coupon model**: Add to Prisma schema:
   - `Coupon`: `id`, `code` (unique string), `type` (percentage or fixed), `value` (discount amount), `vendorId` (nullable — null means global), `minOrderValue` (nullable), `maxUses` (nullable), `currentUses`, `startsAt`, `expiresAt`, `isActive`, `createdAt`
...
7. **Seed data**: Add 2-3 sample coupons (one global percentage, one vendor-specific fixed, one expired)
...
- **Save**: Decimal/money handling approach
